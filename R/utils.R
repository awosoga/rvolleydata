validate_seasons <- function(seasons, start_year) {
  ...
  for (year in seasons) {
    ...
    if (year < start_year || year > current_year) {
      stop(sprintf(
        "Year %d is out of range. Valid range is %d to %d",
        year,
        start_year,
        current_year
      ))
...
start_season <- function(league) {
  switch(league, "mlv" = 2024, "au" = 2022, "lovb" = 2025)
}

internal_name <- function(league) {
  switch(league, "mlv" = "pvf", "au" = "aupvb", "lovb" = "lovb")
}


get_data <- function(league = NULL, seasons) {
  stopifnot(
    "league must be one of 'mlv', 'lovb', or 'au'" = !is.null(league) ||
